...
const FriendsScreen = () => {
  ...
  const route = useRoute<RouteProp<RootStackParamList, "FriendsScreen">>();
    ...
    async function loadFriends() {
      try {
        ...
        if (storedFriends !== null) {
          ...
          await AsyncStorage.setItem("friendsList", JSON.stringify(defaultFriends));
        }
      } catch (error) {
  ...
  // Save friendsList to AsyncStorage whenever it changes
  useEffect(() => {
  ...
  // Check for unfriended phone passed as a param from DetailScreen
  useFocusEffect(
    React.useCallback(() => {
      const unfriendedPhone = route.params?.unfriendedPhone;
      if (unfriendedPhone) {
        setFriendsList((currentList) =>
          currentList.filter((friend) => friend.phone !== unfriendedPhone)
        );
        (route as any).params.unfriendedPhone = undefined;
      }
    }, [route])
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerRow}>
        ...
        <View style={{ flexDirection: "row", gap: 25, alignItems: "center" }}>
          <TouchableOpacity onPress={() => setShowDropdown(true)} style={styles.dropdownToggleMini}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.toggleContainer}>
              <TouchableOpacity
                style={[styles.toggleButton, searchType === "phone" && styles.toggleButtonActive]}
          ...
          <View style={styles.friendItem}>
            ...
            <View style={styles.friendInfo}>
              <Text style={styles.friendName}>
                {item.name} {item.favorited && <Feather name="star" size={16} color="#f0e9af" />}
              </Text>
...
const styles = StyleSheet.create({
  ...
  modalItem: { paddingVertical: 12, paddingHorizontal: 40 },
